...
async def _(event):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await event.edit("```Please unblock me (@AH_File2Link_Bot) u Nigga```")
            return
        await event.delete()
        await event.client.send_message(
            event.chat_id, response.message, reply_to=reply_message
...
@borg.on(admin_cmd(pattern="gone ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
    if not event.reply_to_msg_id:
        ...
        return
    reply_message = await event.get_reply_message()
    chat = "@FiletoLinkTelegramBot"
    reply_message.sender
    await event.edit("**Making public url...**")
    async with event.client.conversation(chat) as conv:
        try:
            response = conv.wait_event(
                events.NewMessage(incoming=True, from_users=1230983246)
            )
            await event.client.forward_messages(chat, reply_message)
            response = await response
        except YouBlockedUserError:
            await event.edit("```Please unblock me (@@FiletoLinkTelegramBot) u Nigga```")
            return
        await event.delete()
        await event.client.send_message(
            event.chat_id, response.message, reply_to=reply_message
